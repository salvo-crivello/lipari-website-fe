...
3. **`components/`** — only what's genuinely cross-page: `layout/`
   ...
   primitives, each in its own folder — see §1), `ui/brand/` (Logo and other
   one-off brand assets, not reusable primitives), `motion/` (FadeIn,
   ScrollReveal, Parallax). No `components/sections/` — that's what
   `views/*/sections/` replaced.
